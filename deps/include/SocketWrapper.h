...
public:
	...
	int recv(SocketWrapperHandle s, char* buf, int len);
	int send(SocketWrapperHandle s, const char* buf, int len);
	int recvfrom(SocketWrapperHandle s, char* buf, int len, unsigned int* farAddr, unsigned short* farPort);
	int sendto(SocketWrapperHandle s, const char* buf, int len, unsigned int farAddr, unsigned short farPort);
