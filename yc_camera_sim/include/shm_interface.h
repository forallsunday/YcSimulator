...
typedef struct SecEnAttrVApp {
    MessageHeader     St_MessageHeader;     // @ID(0)
    EntityAttribute   St_EntityAttribute;   // @ID(1) // 实体姿态
...
// 标识：EOImageShowArea - 显示区域参数数据定义
typedef struct EOImageShowArea {
    float F4_AZLOS_deg_BODY;      // @ID(0) // 视线方位角(deg) [-180,180]
    float F4_ELLOS_deg_BODY;      // @ID(1) // 视线俯仰角(deg) [-90,90]
    float F4_horizontalCover_deg; // @ID(2) // 水平范围(deg) [0,180]
    float F4_verticalCover_deg;   // @ID(3) // 垂直范围(deg) [0,180]
} EOImageShowArea;                // @Extensibility(EXTENSIBLE)

typedef struct EOImageModePara {
    unsigned char U1_ImageMode; // @ID(0) //0-NA，1-区域成像，2-追踪成像，3-回放成像，4-着陆成像，5-模拟平显成像
} EOImageModePara;              // @Extensibility(EXTENSIBLE)

typedef struct EOImageEffect {
    unsigned char U1_VideoSource; // @ID(0) //像源选择 0-NA，1-中波，2-长波，3-可见光
    unsigned char U1_VideoPolar;  // @ID(1) //视频极化 0-NA，1-黑热，2-白热（光雷仅在中波/长波下可进行视频极化）
    unsigned char U1_Contrast;    // @ID(2) //对比度 DAS专用，0-NA，从1开始，根据具体型号确定级数
    unsigned char U1_Gray;        // @ID(3) //灰度 DAS专用，0-NA，从1开始，根据具体型号确定级数
    unsigned char U1_DigitalZoom; // @ID(4) //数字变焦 0-NA,1-1x,2-2x,3-4x
} EOImageEffect;                  // @Extensibility(EXTENSIBLE)

typedef struct EOImageTrackPara {
    EntityID       St_EntityID;     // @ID(0) //追踪目标编号 同战场
    float          F4_TargetDis;    // @ID(1) //显示目标距离 融合距离，单位km，保留小数点后一位
    unsigned short U2_ShowEntityID; // @ID(2) //显示目标编号 融合ID。光雷可跟踪4个目标，优先显示跟踪状态前两位，有丢失情况，自动补齐
...
// 标识：EOImageState - 成像状态数据定义
typedef struct EOImageState {
    unsigned char U1_IRSensor;  // @ID(0) // 传感器状态 0-NA，1-可见光，2-红外，3-可见+红外，4-红外+可见
    unsigned char U1_TVState;   // @ID(1) // 视频状态 0-NA，1-图像，2-视频
    unsigned char U1_ImageMode; // @ID(2) // 成像模式 0-NA，1-广域成像，2-区域成像，3-区域监视
...
typedef struct EOImageParasIS {
    ...
    unsigned int       U4_WideCover;           // @ID(5) // 收容宽度(m)，广域/区域成像模式下垂直航向的实际宽度
    float              F4_ELLOS_deg_BODY;      // @ID(6) // 视场中心俯仰角(deg) [-90,90]
    float              F4_AZLOS_deg_BODY;      // @ID(7) // 视场中心方位角(deg) [-180,180]
    float              F4_verticalCover_deg;   // @ID(8) // 视场大小俯仰(deg) [0,180]
    float              F4_horizontalCover_deg; // @ID(9) // 视场大小方位(deg) [0,180]
    unsigned char      U1_CouseTgtCover;       // @ID(10) // 航向重叠率 [0,99]%，平行飞行方向两帧图像的重叠率
    unsigned char      U1_BesideTgtCover;      // @ID(11) // 傍向重叠率 [0,99]%，垂直飞行方向两帧图像的重叠率
    unsigned short     U2_CurResolution;       // @ID(12) // 像元分辨率(m，LSB=0.01) [00.00,99.99]
    unsigned short     U2_GroundResolution;    // @ID(13) // 地面摄影分辨率(m，LSB=0.01) [00.00,99.99]
    Position           St_ImgCenterPosition;   // @ID(14) // 图像中心经纬高
    Longlat            St_ImgLeftUp;           // @ID(15) // 图像左上角经纬度
    Longlat            St_ImgLeftDown;         // @ID(16) // 图像左下角经纬度
    Longlat            St_ImgRightUp;          // @ID(17) // 图像右上角经纬度
    Longlat            St_ImgRightDown;        // @ID(18) // 图像右下角经纬度
    unsigned short     U2_TakeTimes;           // @ID(19) // 照相次数 [0,999]，每次接收拍照指令或模式变换时+1
    unsigned short     U2_FocalLength;         // @ID(20) // 相机焦距(mm) [0,65535]
    unsigned short     U2_ExposureTime;        // @ID(21) // 曝光时间(us) [0,999]
...
typedef struct EOImageModifyPara {
    unsigned char I1_LightValueLight;     // @ID(0) // 可见光调光值 [-127,127]
    unsigned char I1_FocusValueLight;     // @ID(1) // 可见光调焦值 [-127,127]
    unsigned char U1_LightMistEliminate;  // @ID(2) // 可见光去雾 0-NA,1-ON,2-OFF
    unsigned char U1_LightEnhance_Mode;   // @ID(3) // 可见光增强 0-NA,1-ON,2-OFF
    unsigned char I1_LightValueInfrared;  // @ID(4) // 红外调光值 [-127,127]
    unsigned char I1_FocusValueInfrared;  // @ID(5) // 红外调焦值 [-127,127]
    unsigned char U1_VideoPolar;          // @ID(6) // 极化 0-NA,1-黑热,2-白热
    unsigned char U1_InfraredEnhanceMode; // @ID(7) // 红外增强 0-NA,1-ON,2-OFF
...
typedef struct SimulatorStatusControl {
    unsigned char U1_OperationMode; // @ID(0) // 0- NA；1-初始化；2-快速启动；3-常规启动；4-冻结；5-停止
...
// 标识：EOTgtPositionPara - 动目标像素坐标参数数据定义
typedef struct EOTgtPositionPara {
    unsigned short U2_Tgt1UpleftX;    // @ID(0) // 目标左上角横向像素坐标 [0,65535]，无有效目标时默认填0，以图像左下角为原点
    unsigned short U2_Tgt1UpleftY;    // @ID(1) // 目标左上角纵向像素坐标 [0,65535]，无有效目标时默认填0，以图像左下角为原点
    unsigned short U2_Tgt1DownrightX; // @ID(2) // 目标右下角横向像素坐标 [0,65535]，无有效目标时默认填0，以图像左下角为原点
    unsigned short U2_Tgt1DownrightY; // @ID(3) // 目标右下角纵向像素坐标 [0,65535]，无有效目标时默认填0，以图像左下角为原点
...
typedef struct SecEOImageDriveMsg {
    SM_MessageHeader  St_SMMessageHeader;   ///< @ID(0) 共享内存消息头
    EOImageState      St_EOImageState;      ///< @ID(1) 成像状态
    EOImageShowArea   St_EOImageShowArea;   ///< @ID(2) 显示区域参数
    EOImageParasIS    St_EOImageParasIS;    ///< @ID(3) 图像注释信息
    EOImageModifyPara St_EOImageModifyPara; ///< @ID(4) 光电图像调节参数
    EOTgtPara         Arr_EOTgtPara[5];     ///< @ID(5) 动目标检测成像参数[5] (最多包含5个目标)
} SecEOImageDriveMsg;                       ///< @Extensibility(EXTENSIBLE) 可扩展结构体
...
typedef struct SecTgtPositionParaMsg {
    SM_MessageHeader  St_SMMessageHeader;       // @ID(0) // 共享内存消息头
    EOTgtPositionPara Arr_EOTgtPositionPara[5]; // @ID(1) // 动目标像素坐标参数[5]
...
typedef struct SharedMemoryOutput {
    ...
    // 综合光电成像驱动参数
    SecEOImageDriveMsg m_SecEOImageDriveMsg;
